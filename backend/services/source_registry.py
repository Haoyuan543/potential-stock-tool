...
PUBLIC_DATA_SOURCES: list[dict[str, Any]] = [
    ...
    {
        "id": "twse_company_basic",
        "name": "TWSE listed company basic-data OpenAPI",
        ...
        "method": "json_api",
        "covers": ["listed_company_universe", "industry_code", "company_profile"],
    },
    {
        "id": "tpex_company_basic",
        "name": "TPEx listed company basic-data OpenAPI",
        "category": "market_universe",
        ...
        "freshness": "official open data",
        "method": "json_api",
        "covers": ["otc_company_universe", "industry_code", "company_profile"],
    },
    {
...
def source_metadata_for(source: str = "", tier: str = "", url: str | None = None) -> dict[str, Any]:
    ...
    if selected is None:
        if tier in {"official_mops"}:
            selected = next(item for item in source_catalog() if item["id"] == "mops_material_web")
        elif tier == "exchange_alert":
            selected = next(item for item in source_catalog() if item["id"] == "twse_exchange_alerts")
